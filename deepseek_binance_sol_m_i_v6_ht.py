...
DINGTALK_CONFIG = {
    ...
    'account': os.getenv('DINGTALK_ACCOUNT'),
...
TRADE_CONFIG = {
    ...
    'execution_interval': 10,  # 🆕 新增：执行间隔分钟数（可配置）
...
def send_dingtalk_message(title, message, message_type="info"):
    ...
    try:
        ...
        data = {
            "msgtype": "markdown",
            "markdown": {
                "title": f"[{DINGTALK_CONFIG['account']}]{emoji} {title}",
                "text": full_message
            },
            "at": {
                "isAtAll": False
...
def execute_intelligent_trade(signal_data, price_data):
    ...
    try:
        ...
        if signal_data['signal'] == 'BUY':
            ...
            # 🆕 设置多头止盈止损
            if position_size > 0:
                setup_take_profit_stop_loss('long', position_size, signal_data['take_profit'], signal_data['stop_loss'])
            ...
            # 🆕 设置空头止盈止损
            if position_size > 0:
                setup_take_profit_stop_loss('short', position_size, signal_data['take_profit'], signal_data['stop_loss'])
